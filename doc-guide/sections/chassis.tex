...
struct ChassisConfig {
    DriveMotorConfig motors[CHASSIS_MOTOR_COUNT];
...
private:
    ...
    DriveMotorModule _motorFL, _motorFR, _motorRL, _motorRR;
...
4つの\texttt{DriveMotorModule}は\texttt{ChassisModule}のメンバ変数であり、
...
\begin{enumerate}
    ...
    \item \texttt{\_motorFL(config.motors[0])} — 各モーターを対応するConfigで初期化
\end{enumerate}

初期化リスト構文に慣れないうちは「コンストラクタ引数を受け取って、メンバ変数に渡している」とだけ理解すれば十分です。
コンストラクタ本体では、ポインタ配列\texttt{\_motors[]}に各モーターのアドレスをセットしています。
これにより、ループで全モーターに対して統一的に操作できます。
\end{notebox}
...
\vspace{0.3em}
$output_m = M_{0,m} \times forward + M_{1,m} \times lateral + M_{2,m} \times rotation$
\vspace{0.3em}

\noindent ここで $M$ はホイールパターンに応じた $3 \times 4$ の方向行列であり、行が方向（前進・横移動・回転）、列がモーター（左前・右前・左後・右後）を表す。
