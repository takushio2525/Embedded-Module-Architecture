...
\begin{itemize}
    ...
    \item BLEコールバック（別タスク）→内部バッファ→updateInput()→SystemData
    \item deinit()によるBLEリソースの解放
    \item 双方向通信（RX: updateInput()で受信反映、TX: updateOutput()で送信）
\end{itemize}
\end{patternbox}
...
// updateInput()は入力フェーズで呼ばれる（メインタスク）
...
\subsection{送信の流れ（ロジックフェーズ→updateOutput()）}
...
void applyPattern(SystemData& data) {
    ...
    if (needToRespond) {
        ...
        data.ble.sendRequest = true;  // 次のupdateOutput()で送信される
    }
}
\end{lstlisting}

\begin{notebox}[双方向通信のデータフロー]
\textbf{受信}: BLEコールバック → 内部バッファ → updateInput() → SystemData.ble.rxData → ロジックフェーズ\\
\textbf{送信}: ロジックフェーズ → SystemData.ble.txData → updateOutput() → BLE Notify\\
BleModuleは\texttt{inputModules[]}と\texttt{outputModules[]}の両方に登録する。
受信処理は入力フェーズの\texttt{updateInput()}で、送信処理は出力フェーズの\texttt{updateOutput()}で実行される。
\end{notebox}
...
\textbf{項目} & \textbf{内容} \\
\midrule
戻り値     & \texttt{bool} --- サーバー・サービス作成成功で\texttt{true}、失敗で\texttt{false} \\
動作       & BLEデバイス初期化、GATTサーバー作成、サービス・Characteristic登録、コールバック設定、アドバタイズ開始 \\
失敗条件   & \texttt{BLEDevice::createServer()}またはサービス作成の失敗 \\
\bottomrule
\end{tabularx}
\end{funcblock}
