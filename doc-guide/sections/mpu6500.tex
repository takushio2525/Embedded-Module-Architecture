...
\begin{pointbox}[Configにバスポインタ・バスピンを含めない理由]
\texttt{Mpu6500Config}にはI2Cアドレスとサンプリング設定のみを含め、\texttt{TwoWire*}やバスピンは含めない。
バスポインタをConfigに含めると、\texttt{ProjectConfig.h}でバスインスタンスの\texttt{extern}宣言が
必要になり、インクルード順序の制約が生じる。
I2C SDA/SCLのような共有バスピンは\texttt{ProjectConfig.h}に\texttt{constexpr}単体定数として定義し、
\texttt{setup()}内の\texttt{bus.begin()}で直接参照する。
...
共有バスピン（SDA/SCL）は\texttt{ProjectConfig.h}に\texttt{constexpr}単体定数として定義し、
\texttt{setup()}内の\texttt{bus.begin()}で直接参照する。
\end{notebox}
