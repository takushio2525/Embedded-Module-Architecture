...
| Step | PDF | 内容 |
|------|-----|------|
| **1** | [`docs/01_教科書.pdf`](docs/01_教科書.pdf) | クラスの基礎から3フェーズモデルまで、Q&A形式で段階的に学ぶ |
| **2** | [`docs/02_実装ガイド.pdf`](docs/02_実装ガイド.pdf) | sample/ の実コードを題材に、設計パターンの適用方法を学ぶ |
| **3** | [`docs/03_設計仕様書.pdf`](docs/03_設計仕様書.pdf) | ルール集・API仕様として随時参照する |

- Step 1 は C++クラスに不安がある人は第1章「前提知識」から、理解している人は第2章から
- Step 2 は LedModule（最小構成）→ ButtonModule → ... の順に読む
- Step 3 は通読するより、必要なときに該当セクションを引く使い方を推奨

> LaTeX ソースは `doc-textbook/`・`doc-guide/`・`doc/` にあります。
...
このリポジトリは、AIコーディングアシスタント（Claude Code、GitHub Copilot等）がアーキテクチャに沿ったモジュールコードを正確に生成できるよう設計されている。

| ファイル | 対象 | 役割 |
|---|---|---|
| [ARCHITECTURE.md](ARCHITECTURE.md) | AI・開発者 | アーキテクチャの構造ガイド（設計パターン・ルール・テンプレートの一覧） |
| [CLAUDE.md](CLAUDE.md) | AI | 実装ルールの詳細指示（命名規則・ファイル分割・循環依存回避等をAIが厳密に守るための定義） |

フォーク先のプロジェクトでもこれらのファイルを維持すれば、AIが新規モジュールの追加やリファクタリングを設計パターンどおりに実行できる。
...
├── docs/                  コンパイル済みPDF（学習はここから）
├── doc/                   設計仕様書（LaTeXソース）
├── doc-textbook/          教科書（LaTeXソース）
├── doc-guide/             実装ガイド（LaTeXソース）
...
| DisplayBoardModule | 入出力 | TFTディスプレイ + タッチパネル統合 |
...
| ドキュメント | PDF | LaTeXソース | 内容 |
|---|---|---|---|
| 教科書 | [`docs/01_教科書.pdf`](docs/01_教科書.pdf) | `doc-textbook/` | パターンの段階的な学習（「なぜそうするか」） |
| 実装ガイド | [`docs/02_実装ガイド.pdf`](docs/02_実装ガイド.pdf) | `doc-guide/` | 実コードの解説（「実際にどう書いたか」） |
| 設計仕様書 | [`docs/03_設計仕様書.pdf`](docs/03_設計仕様書.pdf) | `doc/` | アーキテクチャのルール・API仕様（「何を守るか」） |
