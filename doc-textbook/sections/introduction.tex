...
\begin{notebox}[関連ドキュメントとプロジェクト構成]
本リポジトリには2つのプロジェクトと3つのドキュメントがあります。

\textbf{プロジェクト:}
\begin{itemize}
  \item \texttt{sample/} — 設計パターンの構造説明用サンプル（動作未確認）
  \item \texttt{verified/} — 実機で動作確認済みのプロジェクト
\end{itemize}

\textbf{ドキュメント:}
\begin{itemize}
  \item \textbf{教科書}（本書） — パターンの段階的な学習（「なぜそうするか」）
  \item \textbf{実装ガイド}（\texttt{doc-guide/}） — \texttt{sample/}の実コードを題材にした解説（「実際にどう書いたか」）
  \item \textbf{設計仕様書}（\texttt{doc/}） — ルール集・API仕様（「何を守るか」）
\end{itemize}

本書で基礎を学んだら、実装ガイドで実コードの理解を深めてください。設計仕様書はルール集として必要なときに参照する使い方を推奨します。
\end{notebox}

\begin{cautionbox}[サンプルコードについて]
\texttt{sample/} に実装されているモジュール群のコードは、あくまで\textbf{設計パターンを具体的に示すためのサンプル実装}である。
...
\begin{enumerate}
  ...
  \item \textbf{第8章: 実コードを読んでみよう} — LedModuleの実コードを読み解く
  \item \textbf{第9章: 実践} — 温度センサーモジュールを一から作る
  \item \textbf{第10章: 設計のコツ集} — よくあるエラーと実務で役立つTips
\end{enumerate}

最終的に、先ほどのベタ書きコードがどう変わるか？ それは3フェーズモデルの章で明らかになります。
