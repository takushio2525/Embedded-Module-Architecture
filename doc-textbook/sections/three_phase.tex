...
void loop() {
    // 出力が先！
    led.update(systemData);   // 前回のデータで光ってしまう

    // 入力が後
    tempSensor.update(systemData);  // 今回のデータを読む

    applyLogic(systemData);  // 今回のデータで判断するが...
...
\begin{goodexample}{applyLogic()の例}
\begin{lstlisting}
...
第2章では60行のベタ書きでしたが、3フェーズモデルで整理すると次のような構成になります。

\begin{itemize}
  \item \texttt{LedModule} — LEDの制御（約20行）
  \item \texttt{TempSensorModule} — 温度の読み取り（約20行）
  \item \texttt{applyLogic()} — ルール（約5行）
