...
%% ── Q0: リポジトリ全体の構成 ──────────────────
\begin{qabox}{リポジトリ全体の構成はどうなってるの？}

本リポジトリには2つのプロジェクトが含まれています。
...
\toprule
\textbf{ディレクトリ} & \textbf{説明} \\
\midrule
\texttt{sample/} & 設計パターンを具体的なコードで示すためのサンプル。動作未確認 \\
\texttt{verified/} & 実機で動作確認を行ったプロジェクト。フォルダごとダウンロードすればそのまま使える \\
\bottomrule
\end{tabularx}

\vspace{2mm}
各プロジェクトは完全に自己完結したPlatformIOプロジェクトです。フォルダ単体でビルド・書き込みができるようになっています。

\end{qabox}

%% ── Q1: フォルダ構成 ──────────────────────────
\begin{qabox}{プロジェクトのフォルダ構成はどうなってるの？}

各プロジェクト（\texttt{sample/}、\texttt{verified/}）はPlatformIOプロジェクトとして構成されています。ツリー構造を見てみましょう。
...
\texttt{ProjectConfig.h} & 共有バスピン定義 + 全モジュールのConfigインスタンスをまとめて定義する \\
\bottomrule
\end{tabularx}
...
モジュールを追加するときは、\texttt{lib/}にフォルダを作った後、この2ファイルにも追記するのを忘れないでください（実践の章で詳しく手順を示します）。
